...
import time
from robot_analysis.get_shelf_score import *
...
def get_score_hit_compare(url, playback_Id):
    ...
    '''pred_hot_shelves = pd.DataFrame(get_map_shelf_score())
    print(pred_hot_shelves)'''
    pred_hot_shelves=pd.DataFrame(get_map_shelf_score().values())
    print(pred_hot_shelves)
    cross = pd.concat([shelf_hits_df, pred_hot_shelves], sort=True, axis=1)
